    ...
    def record(self, depth=0):
        ...
        if tag in (1, 4, 5):
            ...
            for member, kind, info in zip(names, kinds, infos):
                result[member] = self.primitive(info) if kind == 0 else self.record(depth + 1)
    ...
    with open(filename, "rb") as stream:
        ...
        for block in reader.blocks:
            ...
            if (
                length <= 0
                or total > MAX_BYTES
                or block.get("hasParent")
                or not block.get("deferred")
            ):
        ...
        for item in items[:count]:
            ...
            if (blend is not None and blend["value__"] != 0) or (
                ...
                raise PDNError(
                    "This PDN uses a blend mode the editor does not support. "
            ...
            layers.append(
                RasterLayer(str(resolve(props["name"])), image, bool(props["visible"]), opacity)
            )
...
class _Writer:
    ...
    def write(self, root):
        ...
        for lid, library in [(1, "Data"), (2, "Core")]:
            self.pack("<Bi", 12, lid)
            self.string(
                f"PaintDotNet.{library}, Version=5.112.9563.32325, Culture=neutral, PublicKeyToken=null"
            )
        index = 0
...
def write_pdn(filename, width, height, layers, cancellation=None):
    ...
    version = _object(
        ...
        0,
        *[
            primitive(key, 8, value)
            for key, value in zip(
                ("_Major", "_Minor", "_Build", "_Revision"), (5, 112, 9563, 32325)
            )
        ],
    )
    bitmaps = []
    ...
    for layer in layers:
        block = _object(
            ...
            primitive("hasParent", 1, False),
            primitive("deferred", 1, True),
        )
        surface = _object(
            "PaintDotNet.Surface",
            2,
            primitive("width", 8, width),
            primitive("height", 8, height),
    ...
    try:
        with tempfile.NamedTemporaryFile(dir=path.parent, prefix=".pdn-", delete=False) as stream:
            ...
            thumbnail = base64.b64encode(_thumbnail_png(width, height, layers)).decode("ascii")
            header = (
                f'<pdnImage width="{width}" height="{height}" layers="{len(layers)}" '
